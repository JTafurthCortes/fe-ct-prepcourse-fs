...
function numberOfCharacters(string) {
   ...
   return charArray.reduce((count, char) =>{
...
function capToFront(string) {
   ...
   // Tu código:
   const uppercaseLetters = Array.from(string).filter(char => char === char.toUpperCase());
   const lowercaseLetters = Array.from(string).filter(char => char === char.toLowercase());
   const result = uppercaseLetters.join('') + lowercaseLetters.join('');
   return result;
}
...
function deleteAbc(string) {
   ...
   // Tu código:
   const modifiedString = string.replace(/[abc]/gi, '');
