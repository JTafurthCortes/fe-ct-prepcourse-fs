...
function agregarNumeros(arrayOfNums) {
   ...
   // Tu código:
   return arrayOfNums.reduce((accumulator, current) => accumulator + current, 0);
}
...
function mesesDelAño(array) {
   ...
   for (let i = 0; i < array.length; i++){
      const mes = array [i];
      if (mesesPerdidos.includes(mes)) {
         mesesEncontrados.push(mes);
      }
   }

   if (mesesPerdidos.length === mesesEncontrados.length) {
...
function mayorACien(array) {
   ...
   // Tu código:
   const resultado = array.filter((value) => value > 100);
   return resultado;
}
